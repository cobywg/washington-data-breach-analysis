...
The website export is generated by [`src/export_web_data.py`](src/export_web_data.py). Run it from the project root whenever the source CSV changes:

```bash
python src/export_web_data.py
```

The script writes `web/app/breach-summary.json`, which is imported by the website. This keeps the displayed figures traceable to the source data while avoiding browser-side processing of the full CSV.
...
Records are grouped using the cleaned `EntityState` field. For each state, the export calculates both the number of distinct notification IDs and the sum of `WashingtoniansAffected`. Counting unique IDs ensures that a breach would not be counted more than once if duplicate rows were introduced into the data. In the current dataset, all IDs in the selected period are unique, so the unique-ID count is also equal to the number of notification records.
...
- A reproducible Python-to-JSON data pipeline.
- Unique breach counts and affected-resident totals aggregated by state.
- A React and Mapbox choropleth for reporting years 2022-2025.
- A toggle between breach frequency and Washingtonians affected.
- Hover details, metric-specific legends, and a missing-data disclosure.

The next analytical step is to add charts comparing cyberattack type and industry while retaining the same reporting-year definition and missing-data disclosures.
